...
export function randomName() {
    ...
    function randomItem(array: Array<any>) {
        return array[random(array.length)];
    }

    const count = random(33) + 2,
...
// Parse a Greg sentence and return its corresponding id
export function randomNameParse(sentence: string) {
    const words = sentence.split(" "),
        adjectiveFactor = 32,
        nounFactor = adjectiveFactor * adjectives.length,
        verbFactor = nounFactor * nouns.length,
        adverbFactor = verbFactor * verbs.length,
        count = parseInt(words[0], 10),
        adjective = adjectives.indexOf(words[1]),
        noun = nouns.indexOf(words[2]),
        verb = verbs.indexOf(words[3]),
        adverb = adverbs.indexOf(words[4]);
