...
export const DEFAULT_COMMAND_ARGS: BotStartArgs = {
    name: null,
    format: null,
}
...
/**
 * Arguments for starting a bot.
 */
export interface BotStartArgs extends CommonBotArgs {
}
 ...
 */
export class BotStart implements CommandHandler<BotStartArgs> {
    async execute(ctx: CommandContext<BotStartArgs>): Promise<CommandResult<BotInstanceDescriptor>> {
        const start = Date.now();
        const DEFAULT_ARGS: Partial<BotStartArgs> = {
        };
